...
@section('content')
<div class="grid gap-6 xl:grid-cols-[0.8fr_1.2fr] items-start">
    ...
    <div class="rounded-2xl bg-white border border-slate-200 p-6 shadow-sm">
        <div class="flex items-center justify-between mb-5 pb-3 border-b border-slate-100">
            <h2 class="text-base font-bold text-slate-900 m-0">Restok Terbaru</h2>
            <a href="{{ route('admin.laporan.restok.index') }}" class="text-xs font-semibold text-brand-600 hover:text-brand-700 no-underline">Lihat Laporan Lengkap &rarr;</a>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            @forelse($restok as $item)
                <div class="rounded-xl border border-slate-100 bg-slate-50/50 p-4 hover:bg-slate-50 transition-colors flex flex-col">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            <div class="text-sm font-bold text-slate-900 truncate">{{ $item->buah->nama_buah ?? 'Produk dihapus' }}</div>
                            <div class="text-xs text-slate-500 mt-0.5 truncate">Supplier: <span class="font-medium text-slate-700">{{ $item->supplier }}</span></div>
                        </div>
                        <div class="shrink-0 text-right text-[11px] font-semibold text-slate-400 bg-white px-2 py-1 rounded-md border border-slate-200 whitespace-nowrap">{{ $item->created_at->format('d M Y') }}</div>
                    </div>
                    ...
                    @if($item->catatan)
                        <div class="mt-2 text-xs text-slate-400 italic bg-white p-2 rounded-lg border border-slate-100 line-clamp-2">
                            * {{ $item->catatan }}
                        </div>
                    @endif
                </div>
            @empty
                <div class="sm:col-span-2 rounded-xl border border-dashed border-slate-200 bg-slate-50/30 p-8 text-center text-sm text-slate-400 font-medium">
